  ...
  return {
    ...
    resolveAndVerify() {
      return {
        ...
        feature_id: expected.feature_id, head_sha: expected.head_sha,
        required_checks: expected.required_checks, audit_receipt_sha256: expected.audit_receipt_sha256
      };
    }
  };
}
...
test("admission uses the pinned broker result for exact head, checks, and audit", () => {
  ...
  assert.deepEqual(admitted, {
    schema: reviewSchemas.admission, admitted: true, mutation: "none", scope: "pull_request",
    head_sha: head, request_id: "review-7", audit_receipt_sha256: hash,
...
test("admission is closed, time-bounded, and blocks high findings on PASS", () => {
  ...
  assert.throws(() => validateReviewAdmission(receipt({ findings: [p1] }), authority(), now), /open p0 or p1/);
