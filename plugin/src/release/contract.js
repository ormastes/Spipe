...
export const forbiddenReleaseProjectionPatterns = Object.freeze([
  /git\s+push\s+--tags/i,
  /bookmark\s+set\s+main/i,
...
export function projectionSemanticHash(content) { const unsafe = forbiddenReleaseProjectionPatterns.find((pattern) => pattern.test(content)); if (unsafe) throw new Error(`release projection contains forbidden legacy behavior: ${unsafe}`); const semantics = projectionSemantics(content); if (semantics.length !== releaseProjectionContract.length) { const found = new Set(semantics.map(([id]) => id)); const missing = releaseProjectionContract.map(([id]) => id).filter((id) => !found.has(id)); throw new Error(`release projection is missing normalized contract clauses: ${missing.join(", ")}`); } return digest(semantics); }
