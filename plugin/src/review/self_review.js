...
export const selfReviewSchemas = Object.freeze({ request: "spipe-self-review-request/1", policy_db: "spipe-self-review-policy-db/1", subject_policy: "spipe-self-review-subject-policy/1", path_manifest: "spipe-changed-path-manifest/1", resolution: "spipe-self-review-resolution/1", decision: "spipe-self-review-decision/1", admission_command: "spipe-self-review-admission-command/1", admission: "spipe-self-review-admission/1" });
export const selfReviewRequestFields = Object.freeze(["schema", "repo_id", "pull_request", "session_id", "reviewer_id", "reviewer_provider", "reviewer_model", "higher_model_receipt_digest", "request_id"]);
export const fixedSelfReviewRestrictions = Object.freeze([
  ...
  Object.freeze({ id: "SELF004", description: "content classified as self-review authority, independent of its filename" }),
  Object.freeze({ id: "SELF005", description: "the external self-review policy DB and checked-in authority projections" })
]);

const headerFields = ["schema", "record_type", "default_allow"];
const policyFields = ["schema", "record_type", "policy_id", "effect", "repository_id", "session_id", "reviewer_id", "allow_scopes", "deny_scopes", "issued_by", "issued_at", "not_before", "expires_at", "issuer_key_id", "previous_record_sha256", "signature"];
...
function validateRequest(input) {
  ...
  for (const key of ["session_id", "reviewer_id", "reviewer_provider", "reviewer_model", "request_id"]) safeId(input[key], key);
...
function validateSubjectPolicy(record, index) {
  ...
  for (const key of ["repository_id", "session_id", "reviewer_id"]) if (record[key] !== "") safeId(record[key], `${label}.${key}`);
  ...
  exactFields(record.issued_by, ["type", "id"], `${label}.issued_by`); nonempty(record.issued_by.type, `${label}.issued_by.type`); nonempty(String(record.issued_by.id), `${label}.issued_by.id`);
  safeId(record.issuer_key_id, `${label}.issuer_key_id`); nonempty(record.signature, `${label}.signature`); if (record.previous_record_sha256 !== "" && !SHA256.test(record.previous_record_sha256)) throw new Error(`${label}.previous_record_sha256 has invalid format`);
  ...
  if (notBefore < issued || expires <= notBefore || expires - issued > 86_400_000) throw new Error(`${label} validity must start at/after issuance and expire within 24 hours`);
  return Object.freeze({ ...record, issued_by: Object.freeze({ ...record.issued_by }), allow_scopes: Object.freeze(record.allow_scopes.map((scope, scopeIndex) => validateScope(scope, `${label}.allow_scopes[${scopeIndex}]`))), deny_scopes: Object.freeze(record.deny_scopes.map((scope, scopeIndex) => validateScope(scope, `${label}.deny_scopes[${scopeIndex}]`))) });
}
function recordDigest(record) { return digest({ domain: selfReviewSchemas.subject_policy, record }); }

export function parseSelfReviewPolicy(content) {
  if (typeof content !== "string" || content.trim() === "") throw new Error("self-review policy DB must be non-empty UTF-8 JSONL text"); noLoneSurrogates(content, "self-review policy DB");
  const lines = content.split(/\r?\n/).filter((line) => line.trim() !== ""); let records;
  ...
  const header = parseHeader(records[0]); const policies = records.slice(1).map((record, index) => validateSubjectPolicy(record, index + 2));
  ...
  for (let index = 0; index < policies.length; index += 1) { const expected = index === 0 ? "" : recordDigest(policies[index - 1]); if (policies[index].previous_record_sha256 !== expected) throw new Error(`self-review policy hash chain mismatch at record ${index + 2}`); }
  const policyDbSha256 = digest({ domain: selfReviewSchemas.policy_db, header, policies }); return Object.freeze({ header, policies: Object.freeze(policies), policy_db_sha256: policyDbSha256 });
}
...
function validateResolution(input, request, policy, integrationId, now) {
  ...
  for (const key of ["repo_id", "pull_request", "session_id", "request_id", "reviewer_id", "reviewer_provider", "reviewer_model", "higher_model_receipt_digest"]) if (input[key] !== request[key]) throw new Error(`self-review broker ${key} does not match the request`);
  ...
  const issued = canonicalTimestamp(input.issued_at, "resolution.issued_at"); const expires = canonicalTimestamp(input.expires_at, "resolution.expires_at"); const current = now.getTime(); if (issued > current || current >= expires || expires <= issued || expires - issued > 86_400_000) throw new Error("self-review resolution is not currently valid or exceeds 24 hours");
...
function fixedRestrictions(change, path) {
  ...
  if (change.semantic_flags.includes("self_review_authority")) found.add("SELF004");
  if (/(^|\/)(self[-_]review[^/]*\.(jsonl|json|sdn))$/.test(lower) || lower.startsWith(".spipe/self-review")) found.add("SELF005"); return [...found];
}
function activeMatchingPolicies(policy, request, now) { const current = now.getTime(); return policy.policies.filter((record) => (record.repository_id === "" || record.repository_id === request.repo_id) && (record.session_id === "" || record.session_id === request.session_id) && (record.reviewer_id === "" || record.reviewer_id === request.reviewer_id) && canonicalTimestamp(record.not_before, `${record.policy_id}.not_before`) <= current && current < canonicalTimestamp(record.expires_at, `${record.policy_id}.expires_at`)); }

export function planSelfReviewRequest(input) { const request = validateRequest(input); return Object.freeze({ schema: selfReviewSchemas.request, decision: "pending", authoritative: false, mutation: "none", request, request_digest: digest({ domain: selfReviewSchemas.request, request }), fixed_restrictions: fixedSelfReviewRestrictions, next_action: "submit to an operator-owned MCP server; its pinned broker resolves the live PR head/diff and authenticates the exact higher-model receipt" }); }
export function evaluateSelfReviewPrivilege(input, policy, authority, now = new Date()) {
  const request = validateRequest(input); if (!authority || typeof authority.resolveSelfReview !== "function" || typeof authority.integrationId !== "string" || authority.integrationId === "") throw new Error("self-review broker identity is not configured");
  const resolution = validateResolution(authority.resolveSelfReview(request), request, policy, authority.integrationId, now); const policies = activeMatchingPolicies(policy, request, now); const fullDenies = policies.filter((record) => record.effect === "deny" && record.deny_scopes.length === 0); const constraints = policies.filter((record) => record.effect === "constrain"); const denyScopes = policies.flatMap((record) => record.deny_scopes.map((scope) => ({ policy_id: record.policy_id, scope }))); const pathResults = [];
