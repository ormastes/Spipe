...
function validateSession(input) {
  ...
  if (!SAFE_ID.test(sessionId)) throw new Error("session_id must be one safe identifier");
  if (!WORK_REF.test(branch) || branch.includes("..") || branch.includes("//") || branch.endsWith("/")) throw new Error("work_branch must be one safe owned work/* ref");
  if (!isAbsolute(workspace) || !isAbsolute(mainWorkspace)) throw new Error("workspace paths must be absolute");
  let physicalWorkspace; let physicalMainWorkspace;
  try {
    if (!statSync(workspace).isDirectory() || !statSync(mainWorkspace).isDirectory()) throw new Error("not a directory");
    physicalWorkspace = realpathSync.native(resolve(workspace));
    physicalMainWorkspace = realpathSync.native(resolve(mainWorkspace));
  } catch {
    throw new Error("workspace paths must identify existing physical directories");
  }
  if (physicalWorkspace === physicalMainWorkspace) throw new Error("workspace_path must not identify the physical main worktree");
  if (target !== "main" && !RELEASE_LINE.test(target)) throw new Error("target_ref must be main or release/X.Y");
}

export function planIsolatedSession(input) { validateSession(input); return plan("isolated-session", input, ["unique owned work branch", "existing physical non-main workspace", "exact base and target commits", "policy digest bound"], "register or submit this already-created workspace only through an authorized session provider"); }
...
export function planMainFixDiscovery(input) {
  ...
  if (!Number.isSafeInteger(input.interval_seconds) || input.interval_seconds <= 0 || !Number.isSafeInteger(input.last_scan_epoch) || input.last_scan_epoch < 0 || !Number.isSafeInteger(input.now_epoch) || input.now_epoch < input.last_scan_epoch) throw new Error("scan interval and epochs must be positive and monotonic");
  if (input.now_epoch - input.last_scan_epoch < input.interval_seconds) throw new Error("periodic convergence scan is not due yet");
...
export function planForwardPort(input) {
  for (const key of ["release_fix_commit_sha", "main_base_commit_sha", "evidence_release_fix_commit_sha", "evidence_main_base_commit_sha", "evidence_main_result_commit_sha", "main_result_commit_sha"]) match(input, key, COMMIT);
  for (const key of ["review_receipt_sha256", "forward_port_receipt_sha256", "evidence_sha256"]) match(input, key, SHA256);
  ...
  if (input.release_fix_commit_sha === input.main_base_commit_sha || input.main_result_commit_sha === input.main_base_commit_sha) throw new Error("forward-port revisions must produce a new main result from a distinct release fix");
  if (input.evidence_release_fix_commit_sha !== input.release_fix_commit_sha || input.evidence_main_base_commit_sha !== input.main_base_commit_sha || input.evidence_main_result_commit_sha !== input.main_result_commit_sha) throw new Error("forward-port evidence does not bind release fix, exact main base, and result revisions");
  ...
  return plan("forward-port", input, ["exact release-first fix", "exception and review receipts bound", "isolated main-target branch", "renewed main evidence bound to exact base and result", "protected main integration compare-and-swap"], "submit through protected main integration at the exact main_base_commit_sha; do not push main directly");
}
