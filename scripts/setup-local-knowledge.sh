...
if [ "$MODE" = user ]; then
  ...
  common_route="$DESTINATION/.spipe/common"
  if [ -f "$common_route/package.json" ] && grep -Eq '"name"[[:space:]]*:[[:space:]]*"@simple-lang/spipe"' "$common_route/package.json"; then
    echo "common_route=.spipe/common"
  elif git -C "$DESTINATION" ls-files --stage -- .spipe | grep -q '^160000 '; then
